...
\textbf{Market Data}: Daily stock prices from 30 S\&P 500 constituents covering a \textbf{15-year period from January 1, 2010 to November 30, 2025}, encompassing:
\begin{itemize}
\item Post-financial crisis recovery (2010-2012)
\item Bull market (2013-2019)
...
\textbf{State Dimensionality}: 30 assets $\times$ 23 features = \textbf{690-dimensional state space}

\subsection{Data Splits}

Walk-forward chronological splits to prevent look-ahead bias:
\begin{itemize}
\item \textbf{Train}: Jan 2010 - Dec 2021 (87,477 samples, 2,961 days)
\item \textbf{Val}: Jan 2022 - Dec 2023 (15,030 samples, 501 days)
\item \textbf{Test}: Jan 2024 - Nov 2025 (14,250 samples, 475 days)
\end{itemize}
...
\begin{table*}[t]
\centering
\caption{Performance Comparison on 30 S\&P 500 Stocks (Test Set: 2024-2025)}
\label{tab:results}
...
\textbf{E-NSGA-II + X-MARL} & \textbf{15.82\%} & \textbf{14.49\%} & \textbf{0.82} & \textbf{-17.62\%} & \textbf{2.11\%} & \textbf{1.02} \\
\bottomrule
\end{tabular}
\end{table*}
...
Our method achieves \textbf{excellent tail risk metrics}:
\begin{itemize}
\item \textbf{Best CVaR-95\% (2.11\%)}: 2.3\% better than Equal-Weight (2.16\%)
\item \textbf{Best Max Drawdown (-17.62\%)}: 2.8\% better than Equal-Weight (-18.12\%)
\item \textbf{Lowest Volatility (14.49\%)}: 2.8\% lower than Equal-Weight (14.91\%)
\end{itemize}
...
\caption{3D Pareto Front: Return vs Risk vs Explainability. Points are colored by explainability score (blue=low, yellow=high) using viridis colormap. The Pareto-optimal solutions balance all three objectives, demonstrating the trade-offs between competitive returns, tail risk management, and model interpretability.}
\label{fig:pareto3d}
\end{figure}
...
\begin{enumerate}
\item \textbf{Best Tail Risk}: CVaR 2.11\%, Max DD -17.62\%
\item \textbf{Strong Performance}: 15.82\% return, 0.82 Sharpe, 1.02 Sortino
\item \textbf{Interpretability}: Built-in explainability via $\delta=0.05$
\item \textbf{15-Year Validation}: Robust training on diverse market conditions (2010-2021)
\end{enumerate}
...
\textbf{Key Results on 15-Year Dataset (2010-2025)}:
\begin{itemize}
\item Best CVaR (2.11\%) and Max Drawdown (-17.62\%)
\item Strong performance: 15.82\% return, 0.82 Sharpe ratio
\item Excellent risk-adjusted returns: 1.02 Sortino ratio
\item Robust out-of-sample performance on 2024-2025 test period
\end{itemize}
